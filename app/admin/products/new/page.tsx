...
function AddProductContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only add skinType and hairType if they have values
      if (formData.skinType.length > 0) {
        productData.skinType = formData.skinType
      }
      if (formData.hairType.length > 0) {
        productData.hairType = formData.hairType
      }

      // Add perfume notes if category is parfum
      if (formData.category === "parfum" && (formData.perfumeNotesTop.length > 0 || formData.perfumeNotesHeart.length > 0 || formData.perfumeNotesBase.length > 0)) {
      ...
      toast({
        title: "Erreur",
        description: "Échec de la création du produit. Veuillez réessayer.",
        variant: "destructive",
      })
    } finally {
      setLoading(false)

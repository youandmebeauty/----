  ...
  try {
    ...
    try {
      ...
      if (!coffretSnap.empty) {
        ...
        console.info(`[coffrets revalidate] product ${id} variant ${variantIdx} affected coffrets, revalidated coffrets tag`)
      }
    } catch (err) {
      console.warn("Failed to recompute/update coffrets after variant stock change:", err)
    }
